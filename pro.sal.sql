...
INSERT test_auto_increament (id,username) VALUE (NULL,'D');
INSERT test_auto_increament (id,username) VALUE (DEFAULT,'D');
-- 测试uniqu key
CREATE TABLE table_unicde(
id int unsigned key auto_increment,
username varchar(20) not null unique key
);
insert table_unicde (id ,username) value(1,'战三');
-- 创建用户表
CREATE TABLE IF NOT EXISTS `imooc_user`(
`id` INT UNSIGNED KEY AUTO_INCREMENT COMMENT '用户编码',
`username` VARCHAR(20) NOT NULL UNIQUE COMMENT '用户名',
`password`CHAR(32) NOT NULL UNIQUE COMMENT '密码'
)ENGINE=INNODB CHAR SET=UTF8;
INSERT imooc_user (id,username,password) VALUE(1,'懂得',12345);
